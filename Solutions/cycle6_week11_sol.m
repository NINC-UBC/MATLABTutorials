...
% Matlab will automatically recognize the data as a matrix that you have
...
% instead of assigning variables values in the code. You must check the
...
val=input('Please enter a value between -10 & +10: ');
if (val < -10) || (val >10)
...
val=input('Please enter a value between -10 & +10: ');
while (val < -10) || (val >10)
    disp('Warning, value out of range, please try again')
    val=input('Please enter a value between -10 & +10: ');
end
...
npolys = input('Please enter a number of ploygons: ');
s=size(img);
